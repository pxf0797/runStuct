...
class classTemplate:
    ...
    def createSource(self):
        ...
        cm += ('***********************************************************/\n')
        for func in self.__func_list:
            cm += self.__cTemp.generateFunctionComment2(self.__fileName+'_'+func+'('+'h'+self.__fileName+' cthis)','cthis - '+self.__fileName+'类指针','无',self.__fileName+'类功能函数')
            cm += ('void %s_%s(h%s cthis){\n' %(self.__fileName,func,self.__fileName))
            cm += ('    //TODO\n}\n\n')
        cm += self.__cTemp.generateFunctionComment2('h'+self.__fileName+' '+self.__fileName+'_init('+'h'+self.__fileName+' cthis)','cthis - '+self.__fileName+'类指针','h'+self.__fileName+' - cthis/OOPC_NULL',self.__fileName+'类初始化函数')
        cm += ('h%s %s_init(h%s cthis){\n' %(self.__fileName,self.__fileName,self.__fileName))
        cm += ('    // 功能函数配置\n    //cthis->%s = %s_%s;\n    //TODO\n\n' %(self.__func_list[0],self.__fileName,self.__func_list[0]))
        cm += ('    // 参数配置\n    //TODO\n\n')
        cm += ('    return cthis;\n}\n\n')
        cm += self.__cTemp.generateFunctionComment2('h'+self.__fileName+' '+self.__fileName+'_ctor('+'h'+self.__fileName+' cthis)','cthis - '+self.__fileName+'类指针','h'+self.__fileName+' - cthis/OOPC_NULL',self.__fileName+'类构造函数')
        cm += ('CC(%s){\n' %self.__fileName)
        ...
        cm += ('    //TODO\n')
        cm += ('    return cthis;\n}\n\n')
        cm += self.__cTemp.generateFunctionComment2('h'+self.__fileName+' '+self.__fileName+'_dtor('+'h'+self.__fileName+' cthis)','cthis - '+self.__fileName+'类指针','OOPC_RETURN_DATATYPE - OOPC_TRUE/OOPC_FALSE',self.__fileName+'类析构函数')
        cm += ('CD(%s){\n    return OOPC_TRUE;\n}' %self.__fileName)
        cm += ('\n'*2)
