...
#define SIGNAL_GET_MAP_DEFINE(signal_get,signal_set)  signal_get = signal_set,                      //接收信号进行映射
#define SIGNAL_SET_LEN_ADD_DEFINE(signal_set,len)     +signal_set##Len                                        //发送信号长度叠加
#define SIGNAL_SET_REGISTER(name,signal_set,len)      name##SigSetIndex[signal_set] = signal_set##Len;        //发送信号对应长度数组进行注册
#define SIGNAL_GET_REGISTER(name,signal_get,len)      name##SigGetIndex[signal_get] = signal_get##Len;        //接收信号对应长度数组进行注册
...
enum name##SigGet{\
    list_get(SIGNAL_GET_MAP_DEFINE)\
    name##SigGetMax \
};\
...
SIGNAL_SET_MAP_ENUM(name,list_set);                     \
SIGNAL_GET_MAP_ENUM(name,list_get)
...
void test(void)
{
    // ASYN_SIGNALS_REGISTER(SIGNAL_SET_LIST);
