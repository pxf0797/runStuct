...
// 信号定义
#define SIGNAL_SERV_DEFINE(signal_serv,serv,len)                        signal_serv,                                          // 服务信号名称定义
#define SIGNAL_TRIG_DEFINE(signal_trig,signal_serv,len)                 signal_trig,                                          // 触发信号名称定义
#define SIGNAL_SERV_SERV_DEFINE(signal_serv,serv,len)                   signal_serv##Serv = serv,                             // 服务信号关联服务
#define SIGNAL_TRIG_INDEX_MAP_DEFINE(signal_trig,signal_serv,len)       signal_trig##Map = signal_serv,                       // 触发信号映射服务信号
#define SIGNAL_SERV_REGISTER(name,signal_serv,serv,len)                 name##SigServParamLen[signal_serv] = len; \
                                                                        name##SigServBuff[signal_serv] = (void*)serv;         // 服务信号进行注册
#define SIGNAL_TRIG_REGISTER(name,signal_trig,signal_serv,len)          name##SigTrigIndex[signal_trig] = signal_trig##Map;   // 触发信号进行注册

typedef void (*serv_void)(void);
typedef void (*serv_param)(void * buff, uint16 paramLen);
...
extern uint16 name##SigServParamLen[name##SigServMax];          \
...
uint16 name##SigServParamLen[name##SigServMax];                 \
...
uint16 name##SigTrigIndex[name##SigServMax]
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
