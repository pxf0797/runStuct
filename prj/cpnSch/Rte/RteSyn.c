...
* 其他 : RteSyn类功能函数
***********************************************/
void RteSyn_serv(hRteSyn cthis, uint16 signal, uint8 *buff, uint16 paramLen){
    void *func = cthis->serv_ServBuff[signal];
    if(func)
    {
        if(cthis->serv_paramLen[signal])
        {
            ((serv_param)func)(buff, paramLen);
        }else{
            ((serv_void)func)();
        }
    }
...
* 其他 : RteSyn类功能函数
***********************************************/
void RteSyn_trig(hRteSyn cthis, uint16 signal, uint8 *buff, uint16 paramLen){
    cthis->serv(cthis, cthis->trig_index[signal], buff, paramLen);
}
...
* 其他 : RteSyn类初始化函数
***********************************************/
hRteSyn RteSyn_init(hRteSyn cthis, uint16 *serv_paramLen, void **serv_ServBuff, uint16 *trig_index){
    // 功能函数配置
    //cthis->serv = RteSyn_serv;
    //TODO

    // 参数配置
    cthis->serv_paramLen = serv_paramLen;
