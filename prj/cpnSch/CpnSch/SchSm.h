...
***********************************************/
typedef struct{
    uint16 startTick[CPN_SCH_GROUP_TOTAL_NUMS][32];     // 调度周期起始节拍
    uint16 prdTick[CPN_SCH_GROUP_TOTAL_NUMS][32];       // 周期调度节拍
    uint32 actMask[CPN_SCH_GROUP_TOTAL_NUMS];           // 激活请求任务掩码
    uint32 taskMask[CPN_SCH_GROUP_TOTAL_NUMS];          // 已分配任务掩码
    task taskGroup[CPN_SCH_GROUP_TOTAL_NUMS][32];       // 任务组
} taskGroups;
